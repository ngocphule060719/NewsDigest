...
class FakeNewsRepository : NewsRepository {
    ...
    private var getArticleShouldFail = false
    private var getArticleFailRemaining = 0

    var refreshTopHeadlinesCallCount = 0
        private set
    ...
    fun setGetArticleShouldFail(shouldFail: Boolean) {
        getArticleShouldFail = shouldFail
    }

    fun setGetArticleFailRemaining(count: Int) {
        getArticleFailRemaining = count
    }
    ...
    override suspend fun getArticle(id: String): Result<Article> {
        getArticleCallCount++
        if (getArticleFailRemaining > 0) {
            getArticleFailRemaining--
            return Result.error(IllegalStateException("Get article failed"))
        }
        if (getArticleShouldFail) {
            return Result.error(IllegalStateException("Get article failed"))
        }
